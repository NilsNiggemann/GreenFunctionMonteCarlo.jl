# module ProgressBarLoggerExt
    ...
    function write_log(logger::ProgressBarLogger, i, range, Walkers::AbstractWalkerEnsemble, Observables::Any, reconfiguration)
        ...
        function generate_showvalues(i, Observables,reconfiguration)
            RL = get_reconfigurationList(reconfiguration)
            SurvWalkers = length(unique(RL))
            NW = NWalkers(Walkers)
            () -> [("eloc",Observables.energies[i]), ("w_avg",Observables.TotalWeights[i]), ("Walker survival rate",SurvWalkers/NW) ]
        end
        
        next!(logger.p[],showvalues = generate_showvalues(i, Observables,reconfiguration))
    end
